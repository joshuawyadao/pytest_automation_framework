...
DIV_INT_TEST_CASES = ("div_int, expected_div_int", [
    ("pos_int_pos_int", 2.5),                   # 1.1. Div two positive ints
    ("neg_int_neg_int", 0.5),                   # 1.2. Div two negative ints
    ("pos_int_neg_int", -4.5),                  # 1.3. Div one positive, one negative ints
    ("neg_int_pos_int", -7),                    # 1.4. Div one negative, one positive ints
    ("zero_int_zero_int", ZeroDivisionError),   # 1.5. Div two zeros ints
    ("pos_int_zero_int", ZeroDivisionError),    # 1.6. Div one positive, one zero ints
    ("zero_int_pos_int", 0),                    # 1.7. Div one zero, one positive ints
    ("neg_int_zero_int", ZeroDivisionError),    # 1.8. Div one negative, one zero ints
    ("zero_int_neg_int", 0),                    # 1.9. Div one zero, one negative ints
])

# 1.5. All arithmetic w/ STR inputs (always result in TypeError)
STRING_TEST_CASES = ("arith_str, expected_str", [
    ("char_char", TypeError),       # 2.1. Two chars
    ("str_str", TypeError),         # 2.2. Two strings
    ("char_str", TypeError),        # 2.3. One char, one string
    ("str_char", TypeError),        # 2.4. One string, one char
    ("pos_str_pos_str", TypeError), # 2.5. Two strings of positive ints
    ("neg_str_neg_str", TypeError), # 2.6. Two strings of negative ints
    ("pos_str_neg_str", TypeError), # 2.7. Two strings of one positive, one negative
    ("neg_str_pos_str", TypeError), # 2.8. Two strings of one negative, one positive
])

# 1.6. All arithmetic w/ COMBO inputs (always result in TypeError)
COMBO_TEST_CASES = ("arith_combo, expected_combo", [
    ("pos_int_str", TypeError),     # 3.1. One positive int, one positive string
    ("str_pos_int", TypeError),     # 3.2. One positive string, one positive int
    ("neg_int_str", TypeError),     # 3.3. One neg int, one positive string
    ("str_neg_int", TypeError),     # 3.4. One string, one neg int
    ("zero_int_str", TypeError),    # 3.5. One zero int, one string
    ("str_zero_int", TypeError),    # 3.6. One string, one zero int
])
